...
def create_mutant(original_model):
    ...
    for name, layer in mutated_model.named_children():
        if isinstance(layer, nn.Conv2d):
            # 随机选择新的卷积核大小（例如：3x3, 5x5）
            new_kernel_size = random.choice([(3, 3), (5, 5), (7, 7)])

            # 保持输入通道数和输出通道数不变
            in_channels = layer.in_channels
            out_channels = layer.out_channels

            # 计算新的填充（padding），以保持输入和输出的空间维度一致
            new_padding = (new_kernel_size[0] // 2, new_kernel_size[1] // 2)

            # 创建新的卷积层
            new_layer = nn.Conv2d(in_channels, out_channels, kernel_size=new_kernel_size,
                                  stride=layer.stride, padding=new_padding, bias=False)
            mutated_model._modules[name] = new_layer

            # 确保BatchNorm层的通道数与新卷积层一致
            bn_name = 'bn' + name[-1]
            if bn_name in mutated_model._modules:
                mutated_model._modules[bn_name] = nn.BatchNorm2d(out_channels)
...
# 生成变异体
# 生成变异体
mutants = [create_mutant(model).to(device) for _ in range(3)]
...
for iteration in range(10):
    # 检查模型是否已经在3000个标记样本上进行了训练
    if len(labelled_indices) >= 3000:
        print("Model has been trained on 3000 labelled samples.")
    else:
        print(f"Currently training on {len(labelled_indices)} labelled samples.")
    ...
    # 训练模型
    if len(labelled_indices) >= 3000:
        train_model(model, labelled_loader, criterion, optimizer, epochs=10)

    # 添加打印更新后的标记样本数量
    print(f'Updated labelled samples after training: {len(labelled_indices)}')
    ...
    print(f'Iteration {iteration + 1} - Total labelled samples: {len(labelled_indices)}')
    ...
    for idx, mutant in enumerate(mutants):
        mutant.to(device)  # 这行可以省略，因为已经在生成时处理
    ...
    train_model(model, updated_loader, criterion, optimizer, epochs=10)
