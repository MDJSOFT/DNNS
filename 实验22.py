...
# 随机选择带标记数据，减少数量
num_labelled_samples = 600  # 带标记数据数量%10
...
class SimplifiedVGG16(nn.Module):
    def __init__(self, num_classes=10, image_size=(28, 28, 1)):
        ...
        self.fc1 = nn.Linear(128 * 56 * 56, 512)  # 更新为 128 * 56 * 56
    ...
    def forward(self, x):
        ...
        x = self.maxpool(x)
        #print("After conv1 and maxpool:", x.shape)  # 打印形状
        x = F.relu(self.conv2(x))
        x = self.maxpool(x)
        #print("After conv2 and maxpool:", x.shape)  # 打印形状
        x = torch.flatten(x, 1)
        #print("After flatten:", x.shape)  # 打印形状
        x = F.relu(self.fc1(x))
        x = self.dropout(x)
        x = F.relu(self.fc2(x))
        x = self.fc3(x)
...
model = SimplifiedVGG16().to(device)# 将模型移动到计算设备
...
test_loader = DataLoader(datasets.SVHN(root='./data', split='test', download=True, transform=transform), batch_size=64)
