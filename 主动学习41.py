...
transform = transforms.Compose([
    transforms.ToTensor(),
    transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))  # CIFAR-10 是 RGB 图像
])
...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10):
        ...
        self.in_channels = 16
        self.conv1 = nn.Conv2d(3, 16, kernel_size=3, stride=1, padding=1, bias=False)  # 适应CIFAR-10的3个通道
...
# 定义ResNet-20
def ResNet20():
    return ResNet(BasicBlock, [3, 3, 3, 3], num_classes=10)

# 替换原有的模型定义为 ResNet-20
