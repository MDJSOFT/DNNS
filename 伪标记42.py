...
选择模型对其预测结果不确定性最高的样本。常用指标包括：
最小置信度：选择模型预测置信度最低的样本。
熵：选择模型预测熵最高的样本。
代表性采样 (Representative Sampling)：

选择那些在特征空间中代表性强的样本，确保选择的样本能覆盖数据的不同特征和类别。
边界样本选择 (Boundary Sampling)：

选择位于类别边界附近的样本，这些样本通常最具挑战性，能够推动模型的改进。
集成方法 (Ensemble Methods)：

通过训练多个模型，然后基于它们的预测不确定性选择样本。例如，可以选择不同模型之间预测差异较大的样本。
'''